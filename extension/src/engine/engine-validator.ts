...
type Result = { valid: boolean; reason?: string; engineVersion?: string };
export class EngineValidator {
  private minPatchVersion = MIN_PATCH_VERSION;

  constructor(private readonly extensionVersion: Version) {}

  setMinPatchVersion = (min: number) => {
    this.minPatchVersion = min;
  };

  public validate = async (engineDir: string | undefined): Promise<Result> => {
    if (engineDir === undefined) {
      return { valid: false, reason: 'Provided engine directory is undefined' };
    }
    if (!this.isDirectory(engineDir)) {
      return { valid: false, reason: `Provided engine path is not a directory '${engineDir}'.` };
    }
    return await this.isValidVersion(engineDir);
  };
  ...
  private isValidVersion = async (engineDir: string) => {
    ...
    if (!utilBundleFileName) {
      return { valid: false, reason: `Failed to determine engine version, no util bundle found in '${pluginsDir}'.` };
    }
    const splittedBundleName = utilBundleFileName.split('_');
    if (splittedBundleName.length !== 2 || !splittedBundleName[1]) {
      return { valid: false, reason: `Failed to determine engine version, unexpected util bundle name '${utilBundleFileName}'.` };
    }
    const engineVersion = splittedBundleName[1].replace('.jar', '');
    return this.isValidEngineVersion(engineVersion);
  };
  ...
  private isValidEngineVersion = (engineVersion: string) => {
    ...
    return { valid: true, engineVersion };
  };
