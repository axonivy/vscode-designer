...
export class ProcessBreakpointLocationResolver {
  async lineOfPid(uri: Uri, pid: string): Promise<number | undefined> {
    return (await this.resolve(uri)).pidToLine.get(pid);
  }

  async pidOfLine(uri: Uri, line: number): Promise<string | undefined> {
    return (await this.resolve(uri)).lineToPid.get(line);
  }

  private async resolve(uri: Uri): Promise<ResolvedProcessBreakpointLocations> {
