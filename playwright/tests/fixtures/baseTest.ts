...
export const test = base.extend<{ workspace: string; runtimeWorkspace: string; closeAllTabsOnInit: boolean; page: Page }>({
  workspace: prebuiltWorkspacePath,
  runtimeWorkspace: async ({ workspace }, use) => {
    const tmpWorkspace = await createTmpWorkspace(workspace);
    // eslint-disable-next-line react-hooks/rules-of-hooks
    await use(tmpWorkspace);
    if (runInBrowser || !process.env.CI) {
      await removeTmpWorkspace(tmpWorkspace);
    }
  },
  closeAllTabsOnInit: true,
  page: async ({ runtimeWorkspace, closeAllTabsOnInit }, take) => {
    if (runInBrowser) {
      await runBrowserTest(runtimeWorkspace, closeAllTabsOnInit, take);
    } else {
      await runElectronAppTest(runtimeWorkspace, closeAllTabsOnInit, take);
    }
  }
});

const runBrowserTest = async (runtimeWorkspace: string, closeAllTabsOnInit: boolean, take: (r: Page) => Promise<void>) => {
  ...
  await page.goto(`http://localhost:3000/?folder=${runtimeWorkspace}`);
...
const runElectronAppTest = async (runtimeWorkspace: string, closeAllTabsOnInit: boolean, take: (r: Page) => Promise<void>) => {
